...
function classificationBadge(c: string | null) {
  ...
  if (!s) return null;
  return (
    <span className="flex items-center gap-1 text-2xs font-medium px-2 py-0.5 rounded-full"
...
function KanbanCard({
  ...
  return (
    <div className="bg-background rounded-xl border border-border p-3 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-foreground truncate capitalize">
            {record.business_name}
          </p>
          <p className="text-[11px] text-muted-foreground capitalize">{record.business_type} · {record.business_region}</p>
        </div>
        {channelIcon(record.channel)}
      </div>

      {record.response_text && (
        <p className="text-[11px] text-muted-foreground italic mt-1 mb-2 line-clamp-2 bg-muted rounded-lg p-2">
          "{record.response_text}"
        </p>
      )}
  ...
  return (
    <div className="flex-1 min-w-[220px] max-w-[280px]">
      ...
      <div className="space-y-2 min-h-[120px]">
        ...
        {records.length === 0 && (
          <div className="text-center text-[11px] text-muted-foreground py-8 border-2 border-dashed border-border rounded-xl">
            Sin registros
          </div>
        )}
      </div>
...
function InboxItem({
  ...
  return (
    <div className="bg-background rounded-xl border border-border overflow-hidden">
      <button
        onClick={onToggle}
        className="w-full flex items-center gap-3 p-4 hover:bg-muted focus:outline-none focus:ring-2 focus:ring-primary focus:ring-inset transition-colors text-left"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-foreground capitalize">{record.business_name}</span>
      ...
      {expanded && (
        <div className="border-t border-border p-4 bg-muted space-y-3">
          <div>
            <p className="text-2xs font-semibold text-muted-foreground uppercase tracking-wide mb-1">Mensaje enviado</p>
            <p className="text-xs text-foreground bg-background rounded-lg p-3 border border-border">
              {record.message_text}
            </p>
          </div>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Outreach CRM</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Pipeline de socios fundadores VZ Explorer</p>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href="/admin/outreach/sequences"
            className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg border border-border hover:bg-muted transition-colors text-muted-foreground"
          >
      ...
      {stats && (
        <div className="grid grid-cols-8 gap-3">
          {[
            ...
            <div key={s.label} className="bg-background rounded-xl border border-border p-3 shadow-sm">
