        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input id="sz-name" {...register('name')} placeholder="e.g., Caracas Centro" aria-invalid={!!errors.name} />
              {errors.name && <p className="text-xs text-destructive" role="alert">{errors.name.message}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="sz-level">Safety Level *</Label>
              <Select onValueChange={(v) => setValue('level', v as Form['level'])}>
                <SelectTrigger id="sz-level" aria-invalid={!!errors.level}><SelectValue placeholder="Select level" /></SelectTrigger>
                <SelectContent>
              ...
              {errors.level && <p className="text-xs text-destructive" role="alert">{errors.level.message}</p>}
            </div>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="sz-description">Description *</Label>
            <Textarea id="sz-description" {...register('description')} rows={2} aria-invalid={!!errors.description} />
            {errors.description && <p className="text-xs text-destructive" role="alert">{errors.description.message}</p>}
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="sz-tips">Safety Tips (one per line)</Label>
            <Textarea id="sz-tips" {...register('tips')} rows={3} placeholder="Stay in groups at night&#10;Avoid remote areas" />
          </div>
