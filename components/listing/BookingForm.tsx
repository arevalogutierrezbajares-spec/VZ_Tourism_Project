    ...
    <div className="flex items-center gap-1">
      {STEP_LABELS.map((label, i) => (
        <div key={i} className="flex items-center gap-1">
          <div
            className={`w-6 h-6 rounded-full text-[10px] flex items-center justify-center font-semibold tabular-nums transition-colors duration-150 ease-out ${
              i < current
...
function PriceSummary({
  ...
  return (
    <div className="text-sm space-y-1.5 rounded-lg p-3 bg-muted/30 shadow-[0px_0px_0px_1px_rgba(0,0,0,0.06),0px_1px_2px_-1px_rgba(0,0,0,0.06),0px_2px_4px_0px_rgba(0,0,0,0.04)]">
      ...
      <div className="flex justify-between font-bold text-base border-t pt-1.5 mt-1.5">
        <span>Total</span>
        <span className="tabular-nums">${netTotal.toFixed(2)} USD</span>
      </div>
    </div>
  );
}
...
export function BookingForm({ listing }: BookingFormProps) {
  ...
  return (
    <Card className="shadow-[0px_0px_0px_1px_rgba(0,0,0,0.06),0px_1px_2px_-1px_rgba(0,0,0,0.06),0px_2px_4px_0px_rgba(0,0,0,0.04),0px_8px_16px_-4px_rgba(0,0,0,0.08)] ring-0 sticky top-24">
      ...
      <CardContent className="space-y-4">
        ...
        {step === 'select' && (
          <>
            ...
            {formData.check_in && (
              <div className="text-sm text-muted-foreground p-2 bg-muted/40 rounded-lg tabular-nums">
                {getNights()} night{getNights() > 1 ? 's' : ''} · {formData.guest_count} guest
                {formData.guest_count > 1 ? 's' : ''} · <strong>${getTotal().toFixed(2)}</strong>
              </div>
            )}

            <Button className="w-full active:scale-[0.96] transition-transform duration-150 ease-out" disabled={!canProceedFromSelect} onClick={handleContinueFromDates}>
        ...
        {step === 'details' && (
          <>
            ...
            <div className="flex gap-2">
              ...
              <Button
                className="flex-1 active:scale-[0.96] transition-transform duration-150 ease-out"
        ...
        {step === 'review' && (
          <>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between text-muted-foreground">
                <span>Check-in</span>
                <span className="font-medium text-foreground tabular-nums">{formatDate(formData.check_in)}</span>
              </div>
              <div className="flex justify-between text-muted-foreground">
                <span>Check-out</span>
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1 active:scale-[0.96] transition-transform duration-150 ease-out" onClick={handleBookNow} disabled={isLoading}>
                {isLoading && (
        ...
        {step === 'payment' && (
          <>
                ...
                {PAYMENT_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left flex items-start gap-3 p-3 rounded-lg border-2 transition-colors duration-150 ease-out ${
                        selected
              ...
              (formData.payment_method === 'zelle' || formData.payment_method === 'usdt') && (
                <div className="border border-primary/20 rounded-lg p-3 bg-primary/5 space-y-2 text-sm">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="text-muted-foreground hover:text-foreground p-1 rounded min-w-[40px] min-h-[40px] flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="text-muted-foreground hover:text-foreground flex-shrink-0 p-1 rounded min-w-[40px] min-h-[40px] flex items-center justify-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                  ...
                  <div className="flex justify-between font-semibold">
                    <span>Amount</span>
                    <span className="tabular-nums">{paymentDetails.amount}</span>
                  </div>
                  <div className="flex justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1 active:scale-[0.96] transition-transform duration-150 ease-out" onClick={handlePay} disabled={isLoading}>
                {isLoading && (
                  <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin mr-1.5" aria-hidden="true" />
                )}
                {isLoading
